...
const ScenarioDetail: React.FC = () => {
    ...
    const handleExecute = async () => {
        ...
        try {
            ...
            // Step 1: Fetch "Before" state with params
            await fetchBeforeState(scenarioMeta, paramValues);

            // Step 2: Execute the SQL script by sending raw SQL to server
            const response = await api.executeRawSql(sqlContent, params);
            console.log(response);
            if (response.success) {
                // Step 3: Fetch "After" State with params

                // Fetch separate tables if configured
                if (backendOnline && scenarioMeta.separateTables) {
                    try {
                        const results: Record<string, any[]> = {};
                        for (const table of scenarioMeta.separateTables) {
                            const data = await api.runQuery(table.sqlAfter, params);
                            results[table.name] = data;
                        }
                        setSeparateTablesAfter(results);
                    } catch (e) {
                        console.error("Failed to fetch separate tables after state", e);
                    }
                }

                // Also fetch merged view
                if (backendOnline && scenarioMeta.sqlFetchAfter) {
                    try {
                        const fetchedAfter = await api.runQuery(scenarioMeta.sqlFetchAfter, params);
                        setAfterData(fetchedAfter);
                        setStatus('success');
                        setHasExecuted(true);
                    } catch (e) {
                        setErrorMessage('Execution successful, but failed to fetch result state.');
                        setStatus('error');
                    }
                } else if (scenarioMeta.separateTables) {
                    // If only separate tables, mark as success
                    setStatus('success');
                    setHasExecuted(true);
                } else {
                    // Fallback: Use response data (simulated) or Mock Data
                    const fallbackData = response.data?.after || scenarioMeta.mockData?.after || [];
                    setAfterData(fallbackData);
                    setStatus('success');
                    setHasExecuted(true);
                }
            } else {
                setStatus('error');
                setErrorMessage(response.error || 'Unknown error occurred on server.');
            }
        } catch (err: any) {
        ...
        <div className="flex flex-col h-full bg-slate-50 dark:bg-dark-bg">
            ...
            <div className="flex-1 flex flex-col p-2 gap-2 overflow-hidden">
              ...
              `}
                ...
                {!isTablesCollapsed && (
                    <>
                        {/* ROW 2: Before & After Tables - Only show when NOT executed */}
                        {!hasExecuted && (
                            <div className="flex-1 min-h-[200px] overflow-y-auto">
                                ...
                                {/* Show separate tables with initial data */}
                                {scenarioMeta.separateTables ? (
                                    <div className="flex flex-col h-full">
                                        {/* Tabs Header with Collapse Button */}
                                            ...
                                            <div className="flex gap-1">
                                                {scenarioMeta.separateTables.map((tableDef: any) => (
                                                    <button
                                                        ...
                                                        className={`
                                            px-4 py-2 rounded-t text-sm font-medium transition-all
                                            ${activeTableTab === tableDef.name
                                        ...
                                        `}
                                            ...
                                            <button
                                                ...
                                                className="px-3 py-1.5 rounded text-xs font-semibold transition-all bg-orange-500 hover:bg-orange-600 text-white shadow-sm"
                                            >
                                                {isTablesCollapsed ? '👁️ Show Tables' : '🙈 Hide Tables'}
                                            </button>
                                        </div>
                                                ...
                                                {scenarioMeta.separateTables.map((tableDef: any) => (
                                                    activeTableTab === tableDef.name && (
                                                        <TableSnapshot
                                                            ...
                                                            headerColor="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400"
                                    ...
                                    <>
                                        {!isTablesCollapsed && (
                                            <div className="relative">
                                                {/* Collapse Button for non-separateTables scenarios */}
                                                <div className="absolute top-2 right-2 z-10">
                                                    <button
                                                        onClick={() => setIsTablesCollapsed(!isTablesCollapsed)}
                                                        className="px-3 py-1.5 rounded text-xs font-semibold transition-all bg-orange-500 hover:bg-orange-600 text-white shadow-md"
                                                    >
                                                        🙈 Hide Tables
                                                    </button>
                                                </div>
                                                <TableSnapshot
                                                    ...
                                                    title="CURRENT TABLE STATE (Full Data)"
                                                    headerColor="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400"
                                                    className="border-blue-300 dark:border-blue-800"
                                                />
                                            </div>
                                        ...
                                        {isTablesCollapsed && (
                                            <div className="flex items-center justify-center py-8">
                                                <button
                                                    onClick={() => setIsTablesCollapsed(false)}
                                                    className="px-4 py-2 rounded text-sm font-semibold transition-all bg-orange-500 hover:bg-orange-600 text-white shadow-md"
                                                >
                                                    👁️ Show Tables
                                                </button>
                                            </div>
                        ...
                        {/* ROW 3: Diff View - Show full screen when executed */}
                        {hasExecuted && (
                            <div className="flex-1 min-h-[200px] flex flex-col relative">
                                {/* Connecting Arrow - only show after execution */}
                                {hasExecuted && (
                                    <div className="absolute left-1/2 -top-3 -translate-x-1/2 z-10 text-slate-300 dark:text-slate-600">
                                        <ArrowDown size={20} />
                                    </div>
                                )}

                                {hasExecuted && status === 'success' ? (
                                    scenarioMeta.separateTables && Object.keys(separateTablesBefore).length > 0 ? (
                                        <div className="h-full flex flex-col gap-2 overflow-y-auto rounded-lg bg-white dark:bg-dark-surface p-2">
                                            {scenarioMeta.separateTables.map((tableDef: any) => {
                                                const tablePk = tableDef.columns.find((c: any) => c.isPk)?.key || tableDef.columns[0]?.key;
                                                return (
                                                    <div key={tableDef.name} className="flex-shrink-0">
                                                        <div className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1 px-2">
                                                            {tableDef.label} - DIFF
                                                        </div>
                                                        <DiffView
                                    ...
                                    <div className="h-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-dark-surface flex items-center justify-center">
                                        <div className="text-center text-slate-400">
                                            {!hasExecuted ? (
                                                <>
                                                    <p className="text-xs font-medium mb-1">💡 Initial View</p>
                                                    <p className="text-[10px]">Showing full table data above</p>
                                                    <p className="text-[10px]">Execute to see before/after comparison</p>
                                                </>
                                            ) : (
                                                <p className="text-xs font-medium">Difference analysis pending...</p>
                                            )}
                                        </div>
                                    </div>
                                )}
