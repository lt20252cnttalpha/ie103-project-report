...
#table(
  columns: (1fr,) * 4,
  align: (left, left, left, left),
  [Thuộc Tính], [Kiểu Dữ Liệu], [Ràng Buộc], [Mô Tả], [#strong[id]], [`INT`], [`PK`, `IDENTITY`], [Khóa chính.], [code], [`NVARCHAR(50)`], [`NOT NULL`, `UNIQUE`], [Mã vai trò (VD: ADMIN).], [name], [`NVARCHAR(255)`], [`NOT NULL`], [Tên hiển thị.], [description], [`NVARCHAR(500)`], [`NULL`], [Mô tả vai trò.]
)
...
#table(
  columns: (1fr,) * 4,
  align: (left, left, left, left),
  [Thuộc Tính], [Kiểu Dữ Liệu], [Ràng Buộc], [Mô Tả], [#strong[id]], [`INT`], [`PK`, `IDENTITY`], [Khóa chính.], [code], [`NVARCHAR(100)`], [`NOT NULL`, `UNIQUE`], [Mã quyền (VD: USER\_READ).], [description], [`NVARCHAR(255)`], [`NULL`], [Mô tả quyền hạn.]
)
...
#table(
  columns: (1fr,) * 4,
  align: (left, left, left, left),
  [Thuộc Tính], [Kiểu Dữ Liệu], [Ràng Buộc], [Mô Tả], [#strong[admin\_id]], [`INT`], [`PK`, `FK` (ADMINS)], [Khóa ngoại + Khóa chính.], [#strong[role\_id]], [`INT`], [`PK`, `FK` (ROLES)], [Khóa ngoại + Khóa chính.], [#emph[FK Rule]], [], [`ON DELETE CASCADE`], [Xóa Admin/Role tự động xóa dòng này.]
)
...
#table(
  columns: (1fr,) * 4,
  align: (left, left, left, left),
  [Thuộc Tính], [Kiểu Dữ Liệu], [Ràng Buộc], [Mô Tả], [#strong[role\_id]], [`INT`], [`PK`, `FK` (ROLES)], [Khóa ngoại + Khóa chính.], [#strong[permission\_id]], [`INT`], [`PK`, `FK` (PERMISSIONS)], [Khóa ngoại + Khóa chính.], [#emph[FK Rule]], [], [`ON DELETE CASCADE`], [Xóa Role/Permission tự động xóa dòng này.]
)
